...
# Single chat messages list success response serializer
class SingleChatMessagesListSuccessResponseSerializer(GenericResponseSerializer):
    """Single chat messages list success response serializer.

    This serializer defines the structure of the single chat messages list success response.
    It includes a status code and a list of messages.

    Attributes:
        status_code (int): The status code of the response (200 OK).
        messages (list): A list of messages in the single chat.
    """
    ...
    messages = MessageResponseSchema(
        many=True,
        read_only=True,
        help_text=_("List of messages in the single chat."),
    )


# Single chat messages list missing parameter response serializer
class SingleChatMessagesListMissingParamResponseSerializer(GenericResponseSerializer):
    """Single chat messages list missing parameter response serializer.
...
# Single chat messages list authentication error response serializer
class SingleChatMessagesListAuthErrorResponseSerializer(GenericResponseSerializer):
    """Single chat messages list authentication error response serializer.
...
# Single chat messages list not found response serializer
class SingleChatMessagesListNotFoundResponseSerializer(GenericResponseSerializer):
    """Single chat messages list not found response serializer.
    ...
    error = serializers.CharField(
        ...
        read_only=True,
        help_text=_("Error message. Could be 'Single chat not found.' or 'No messages found in this chat.'"),
    )


# Single chat messages list permission denied response serializer
class SingleChatMessagesListPermissionDeniedResponseSerializer(GenericResponseSerializer):
    """Single chat messages list permission denied response serializer.
