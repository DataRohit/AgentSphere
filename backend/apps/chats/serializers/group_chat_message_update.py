...
# GroupChat message update serializer
class GroupChatMessageUpdateSerializer(serializers.ModelSerializer):
    """GroupChat message update serializer.

    This serializer handles updating existing messages in a group chat.
    ...
    # Meta class for GroupChatMessageUpdateSerializer configuration
    class Meta:
        """Meta class for GroupChatMessageUpdateSerializer configuration.
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        # Get the group chat from the message
        ...
        # If the message is from a user and the user is not the chat creator
...
# GroupChat message update success response serializer
class GroupChatMessageUpdateSuccessResponseSerializer(GenericResponseSerializer):
    """GroupChat message update success response serializer.

    This serializer defines the structure of the group chat message update success response.
...
# GroupChat message update error response serializer
class GroupChatMessageUpdateErrorResponseSerializer(GenericResponseSerializer):
    """GroupChat message update error response serializer.

    This serializer defines the structure of the group chat message update error response.
    It includes a status code and error details.

    Attributes:
        status_code (int): The status code of the response.
        errors (GroupChatMessageUpdateErrorsDetailSerializer): The validation errors.
    """
    ...
    class GroupChatMessageUpdateErrorsDetailSerializer(serializers.Serializer):
        """GroupChat Message Update Errors detail serializer.
    ...
    errors = GroupChatMessageUpdateErrorsDetailSerializer(
        help_text=_("Validation errors for the group chat message update request."),
    )


# GroupChat message authentication error response serializer
class GroupChatMessageUpdateAuthErrorResponseSerializer(GenericResponseSerializer):
    """GroupChat message authentication error response serializer.
...
class GroupChatMessageUpdateNotFoundErrorResponseSerializer(GenericResponseSerializer):
    """GroupChat message not found error response serializer.
...
class GroupChatMessageUpdatePermissionDeniedResponseSerializer(GenericResponseSerializer):
    """GroupChat message permission denied response serializer.
