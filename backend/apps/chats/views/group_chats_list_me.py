...
# Group chats list me view
class GroupChatsListMeView(APIView):
    """Group chats list me view.

    This view allows authenticated users to list their own group chats within an organization.
    It requires the organization_id parameter and returns only group chats created by the current user.
    Additional filters for is_public are available.
    ...
    @extend_schema(
        ...
        Additional filter for is_public is available.
        """,
        parameters=[
            OpenApiParameter(
    ...
    def get(self, request: Request) -> Response:
        """List group chats created by the current user within an organization.

        This method lists group chats created by the current user within the specified organization.
        The organization_id parameter is mandatory.
        ...
        try:
            ...
            # Initialize queryset to only include group chats created by the current user
