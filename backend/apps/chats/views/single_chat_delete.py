...
from apps.chats.tasks import delete_single_chat_messages
...
# SingleChat delete view
class SingleChatDeleteView(APIView):
    """SingleChat delete view.

    This view allows authenticated users to delete a single chat by ID.
    ...
    def handle_exception(self, exc: Exception) -> Response:
        """Handle exceptions for the single chat delete view.

        This method handles exceptions for the single chat delete view.
    ...
    @extend_schema(
        ...
        """,
        responses={
            status.HTTP_200_OK: SingleChatDeleteSuccessResponseSerializer,
            status.HTTP_401_UNAUTHORIZED: SingleChatDeleteAuthErrorResponseSerializer,
            status.HTTP_403_FORBIDDEN: SingleChatDeletePermissionDeniedResponseSerializer,
            status.HTTP_404_NOT_FOUND: SingleChatDeleteNotFoundResponseSerializer,
        },
    )
    def delete(self, request: Request, single_chat_id: str) -> Response:
        """Delete a single chat by ID.

        This method deletes a single chat by ID. The user must be the owner of the chat or
        the owner of the organization that the chat belongs to.
        All associated messages will also be deleted.

        Args:
            request (Request): The HTTP request object.
            single_chat_id (str): The ID of the single chat to delete.
        ...
        try:
            ...
            if user not in (single_chat.user, single_chat.organization.owner):
            ...
            chat_id = str(single_chat.id)

            # Delete the chat
            single_chat.delete()

            # Delete associated messages using Celery task
            delete_single_chat_messages.delay(
