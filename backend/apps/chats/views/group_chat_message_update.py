...
# GroupChat message update view
class GroupChatMessageUpdateView(APIView):
    """GroupChat message update view.

    This view allows authorized users to update messages in a group chat.
    ...
    @extend_schema(
        ...
        summary="Update a message in a group chat.",
        description="""
        Updates an existing message in a group chat.
        Only the user who created the chat and the owner of the organization can update messages.
        The user can only update user messages, while the organization owner can update both user and agent messages.
        Only the content field can be updated.
        """,
        request=GroupChatMessageUpdateSerializer,
        responses={
            status.HTTP_200_OK: GroupChatMessageUpdateSuccessResponseSerializer,
            status.HTTP_400_BAD_REQUEST: GroupChatMessageUpdateErrorResponseSerializer,
            status.HTTP_401_UNAUTHORIZED: GroupChatMessageUpdateAuthErrorResponseSerializer,
            status.HTTP_403_FORBIDDEN: GroupChatMessageUpdatePermissionDeniedResponseSerializer,
            status.HTTP_404_NOT_FOUND: GroupChatMessageUpdateNotFoundErrorResponseSerializer,
        },
    )
    def patch(self, request: Request, group_chat_id: str, message_id: str) -> Response:  # noqa: PLR0911
        """Update a message in a group chat.

        This method updates an existing message in a group chat.
        Only the user who created the chat and the owner of the organization can update messages.
        The user can only update user messages, while the organization owner can update both user and agent messages.
        Only the content field can be updated.
        ...
        try:
            ...
            try:
                ...
                serializer = GroupChatMessageUpdateSerializer(
                    instance=message,
                    data=request.data,
