...
export function DeleteChatDialog({ open, onOpenChange, chat, onSuccess }: DeleteChatDialogProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            const errorMessage =
                err instanceof Error ? err.message : "An error occurred while deleting the chat";
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px] bg-(--background) border-(--border) [&_[data-slot=dialog-close]]:hover:opacity-100 [&_[data-slot=dialog-close]]:cursor-pointer [&_[data-slot=dialog-close]]:transition-opacity [&_[data-slot=dialog-close]]:duration-200">
                ...
                <div className="py-4">
                    <p className="text-sm font-medium mb-2">You are about to delete the chat:</p>
                    <div className="p-3 rounded-md bg-(--secondary) text-sm">{chat.title}</div>
                </div>
