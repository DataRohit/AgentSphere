    ...
    const handleReject = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/organizations/transfer/${transferId}/reject/`,
                {
                    method: "GET",
                    headers: {
            ...
            if (!response.ok) {
                throw new Error(data.error || "Failed to reject transfer request");
            }
            ...
            toast.error(
                error instanceof Error ? error.message : "Failed to reject transfer request",
                {
                    style: {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px] bg-(--background) border-(--border) [&_[data-slot=dialog-close]]:hover:opacity-100 [&_[data-slot=dialog-close]]:cursor-pointer [&_[data-slot=dialog-close]]:transition-opacity [&_[data-slot=dialog-close]]:duration-200">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Are you sure you want to reject this ownership transfer request?
                    </DialogDescription>
                </DialogHeader>

                <div className="py-4">
                    <p className="text-sm text-center font-medium">
                        You are about to reject the ownership transfer for{" "}
