...
class UserReactivationConfirmView(APIView):
    ...
    def post(self, request: Request, uid: str, token: str) -> Response:
        """Handle POST request for account reactivation confirmation.

        This method processes the confirmation of account reactivation.
        ...
        try:
            try:
                # Get the activation token from the database
                activation_token = get_object_or_404(
                    UserActivationToken.objects.select_related("user"),
                    uid=uid,
                    token=token,
            ...
            # Create serializer instance with request data
            serializer = UserReactivationConfirmSerializer(data=request.data)
...
class UserDeletionConfirmView(APIView):
    ...
    def delete(self, request: Request, uid: str, token: str) -> Response:
        """Handle DELETE request for account deletion confirmation.

        This method processes the confirmation of account deletion.
        ...
        try:
            ...
            # Store user email before deletion for sending farewell email
            user_email = user.email
            user_full_name = user.full_name

            # Get deletion token to delete it
            deletion_token_id = deletion_token.id
            ...
            with transaction.atomic():
                ...
                # Manually delete the token since CASCADE might not work if user is already deleted
                UserDeletionToken.objects.filter(id=deletion_token_id).delete()
...
class UserPasswordResetConfirmView(APIView):
    ...
    @extend_schema(
        ...
        description="""
        Confirms password reset for a user account using the UID and token from the password reset link.
        Requires setting a new password.
        """,
    ...
    def post(self, request: Request, uid: str, token: str) -> Response:
        ...
        This method processes the confirmation of password reset.
